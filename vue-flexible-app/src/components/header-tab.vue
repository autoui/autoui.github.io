...
<!--       <div class="parent-tabs-area">
      ...
      <iscroll-view ref="scrollView" class="scroll-view sub-tabs-area" :options="{click: true, scrollX: true}" :scrollerClass="{'scroller':true}">
          ...
          <span class="tab-item" v-for="(item, index) in config.sub.arr" v-bind:class="{active: index === config.sub.activeIndex, 'has-not-read-msgs': item.notReadNum > 0}">{{ item.cn_name }}</span>
      </iscroll-view>
